...
export function registerAgentCommands(program: Command) {
  ...
  program
    ...
    .option(
      "--wakeup <schedule>",
      'Periodic wakeup schedule: duration ("30s", "5m", "2h") or cron expression ("0 9 * * 1-5")',
    )
    .option("--prompt <text>", "Custom wakeup prompt (used with --wakeup)")
...
Examples:
  ...
  $ agent-worker new monitor --wakeup 30s --prompt "Check status"
  $ agent-worker new reporter --wakeup "0 9 * * 1-5" --prompt "Generate daily report"
      `,
    )
    .action(async (name, options) => {
      ...
      if (options.json) {
        ...
        const parts = [name, `(${model})`];
        if (schedule) {
          parts.push(`wakeup=${schedule.wakeup}`);
        }
