...
import { parseDuration } from "../daemon/registry.ts";
import { parseCron } from "../daemon/cron.ts";
...
/**
 * Validate a wakeup schedule value.
 * Must be a duration string ("30s", "5m", "2h") or a valid cron expression.
 */
function validateWakeup(path: string, value: unknown, errors: ValidationError[]): void {
  if (typeof value !== "string") {
    errors.push({
      path: `${path}.wakeup`,
      message: 'Field "wakeup" must be a string (duration like "30s" or cron expression)',
    });
    return;
  }
  ...
  if (ms !== null) {
    if (ms <= 0) {
      errors.push({
        path: `${path}.wakeup`,
        message: "Wakeup duration must be positive",
      });
    }
    return;
  }
  ...
  try {
    ...
    errors.push({
      path: `${path}.wakeup`,
      message: `Invalid wakeup value "${value}": must be a duration ("30s", "5m", "2h") or cron expression ("0 9 * * 1-5")`,
    });
  }
}
...
function validateAgent(name: string, agent: unknown, errors: ValidationError[]): void {
  ...
  // Validate wakeup (optional)
  if (a.wakeup !== undefined) {
    validateWakeup(path, a.wakeup, errors);
  }

  // Validate prompt (optional, only meaningful with wakeup)
