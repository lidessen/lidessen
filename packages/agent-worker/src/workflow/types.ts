...
export interface AgentDefinition {
  ...
  /**
   * Periodic wakeup schedule: duration string ("30s", "5m", "2h") or cron expression ("0 9 * * 1-5").
   * When set, the agent wakes up periodically and receives the wakeup prompt.
   */
  wakeup?: string;

  /** Custom wakeup prompt sent when the agent wakes up on schedule. */
  prompt?: string;
}
